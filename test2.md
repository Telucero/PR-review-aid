...
print(f":white_check_mark: Validation passed: {result.validation_passed}")
print(f":memo: Call ID: {result.call_id}")
print(f":speech_balloon: Validated output: {result.validated_output}")
...
Expected output:
```console
:white_check_mark: Validation passed: True
:memo: Call ID: 4995163360
:speech_balloon: Validated output: The capital of France is Paris.
:x: Validation failed: Validation failed for field with errors: Potential hallucination detected: The user asked for the capital of France.
...
print(f":white_check_mark: Validation passed: {result.validation_passed}")
print(f":memo: Call ID: {result.call_id}")
print(f":page_facing_up: Raw LLM output: {result.raw_llm_output}")
print(f":sparkles: Validated output: {result.validated_output}")
```
Expected output:
```console
:white_check_mark: Validation passed: True
:memo: Call ID: 4687733984
:page_facing_up: Raw LLM output: The invoice date is May 22, 2025
:sparkles: Validated output: The invoice date is May 22, 2025
```
